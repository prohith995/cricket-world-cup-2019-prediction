# This file extracts the data from web for all the players required
# Time taken for running ~several minutes (Not advised to run this for testing)
...
# Required Data
players_urls_df = read.csv("Urls_Players.csv")
players_urls = list(as.character(players_urls_df$x))[[1]]
all_players_df = data.frame()

# Looping over all the players
for (index_player in c(1:length(players_urls))){
  print(paste("Player",index_player,"out of",nrow(players_urls_df), sep=" "))
...
#write.csv(all_players_df, "Data_Players.csv")
